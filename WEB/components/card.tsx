import Image from "next/image"
import Link from "next/link"
import { IoPeopleOutline, IoLocationOutline } from "react-icons/io5"

const Card = () => {
    return (
        <div className="bg-white shadow-lg rounded-sm transition duration-100 hover:shadow-sm">
            <div className="h-[260px] w-auto rounded-t-sm relative">
                <Image src="/standar-room.jpeg" width={384} height={256} alt="room image" className="w-full 
                h-full object-cover rounded-t-sm" />
            </div>
            <div className="p-8">
                {/* Nama Hotel dan Lokasi */}
                <div className="mb-4">
                    <h4 className="text-2xl font-medium">
                        <Link href="#" className="hover:text-gray-800 transition duration-150">
                            ASTON Lampung Hotel
                        </Link>
                    </h4>
                    <div className="flex items-center text-gray-500 text-sm mt-1">
                        <IoLocationOutline className="mr-1" />
                        <span>Jl. Wolter Monginsidi No.175, Gedong Meneng</span>
                    </div>
                </div>

                <h4 className="text-2xl mb-7">
                    <span className="font-semibold text-gray-600">Rp 450.000</span>
                    <span className="text-gray-400 text-sm">/malam</span>
                </h4>
                <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-2">
                        <IoPeopleOutline />
                        <span>2 Orang</span>
                    </div>
                    <Link href="#" className="px-6 py-2.5 md:px-10 md:py-3 font-semibold text-white
                    bg-[#C2A895] rounded-sm hover:bg-[#b39683] transition duration-150">Pesan Sekarang</Link>
                </div>
            </div>
        </div>
    )
}

export default Card